...
#if !defined(__WIN32__) && !defined(_AIX) && !defined(__NetBSD__) && !(defined(__ANDROID__) && (__ANDROID_API__ < 23)) && !defined(ESP_PLATFORM)
int h_errno;
#endif
...
/* ESP32/lwIP: gethostbyname is available, but gethostbyaddr is not */
struct hostent *ei_gethostbyname(const char *name)
{
    return gethostbyname(name);
}
...
struct hostent *ei_gethostbyaddr(const char *addr, int len, int type)
{
    /* lwIP doesn't provide gethostbyaddr - return NULL */
    /* This is fine since we use direct IP connections */
...
{
    /* lwIP doesn't provide gethostbyaddr - return NULL */
    (void)addr; (void)length; (void)type;
    (void)hostp; (void)buffer; (void)buflen;
    if (h_errnop) *h_errnop = 1; /* HOST_NOT_FOUND */
    return NULL;
}
...
struct hostent *ei_gethostbyname_r(const char *name,
				    ...
				    int *h_errnop)
{
    /* lwIP has gethostbyname but not _r version - use non-reentrant */
    struct hostent *result = gethostbyname(name);
    if (result && hostp) {
        *hostp = *result;
    }
    if (h_errnop) *h_errnop = result ? 0 : 1;
    return result;
}
