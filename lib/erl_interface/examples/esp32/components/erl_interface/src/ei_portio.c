...
static int ei_tcp_get_fd(void *ctx, int *fd)
{
    return EI_DFLT_CTX_TO_FD__(ctx, fd);
}

static int ei_tcp_hs_packet_header_size(void *ctx, int *sz)
{
    int fd;
    *sz = 2;
    return EI_DFLT_CTX_TO_FD__(ctx, &fd);
...
static int ei_tcp_handshake_complete(void *ctx)
{
    int res, fd, one = 1;
...
static int ei_tcp_socket(void **ctx, void *setup_ctx)
{
...
static int ei_tcp_close(void *ctx)
{
    int fd, res;
...
static int ei_tcp_listen(void *ctx, void *addr, int *len, int backlog)
{
    int res, fd;
...
static int ei_tcp_accept(void **ctx, void *addr, int *len, unsigned unused)
{
    int fd, res;
...
static int ei_tcp_connect(void *ctx, void *addr, int len, unsigned unused)
{
    int res, fd;
...
static int ei_tcp_writev(void *ctx, const void *viov, int iovcnt, ssize_t *len, unsigned unused)
{
...
static int ei_tcp_write(void *ctx, const char* buf, ssize_t *len, unsigned unused)
{
    int error, fd;
    ssize_t res;
...
static int ei_tcp_read(void *ctx, char* buf, ssize_t *len, unsigned unused)
{
    int error, fd;
    ssize_t res;
...
ei_socket_callbacks ei_default_socket_callbacks = {
    ...
    ei_tcp_hs_packet_header_size,
    ei_tcp_handshake_complete,
    ei_tcp_handshake_complete,
    ei_tcp_get_fd
